...
const renderStars = (rating: number | null | undefined) => {
  // Ensure rating is a number; default to 0 if null/undefined
  const safeRating = (typeof rating === 'number') ? rating : 0;
  
  return (
    <div className="flex text-amber-400 items-center gap-0.5">
        <span className="text-slate-700 font-bold text-sm mr-1">{safeRating.toFixed(1)}</span>
...
const PlaceCard: React.FC<PlaceCardProps> = ({ place, onDelete, isSelected, isHovered, onHover, onClick, id }) => {
  ...
  return (
    <div 
      ...
      `}
    >
      {/* Cover Image Area - Hidden on Mobile */}
      <div className="hidden sm:block w-full h-44 overflow-hidden relative bg-slate-100">
         {place.imageUri ? (
             <img 
             src={place.imageUri} 
             alt={place.name} 
         ...
         {/* Badges - Hidden on mobile because parent is hidden */}
         <div className="absolute top-2 left-2 sm:top-3 sm:left-3 flex gap-2">
            <span className={`px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full text-[10px] font-bold tracking-wide uppercase shadow-sm ${getCategoryBadgeStyle(place.category)}`}>
                {place.subCategory}
            </span>
         </div>

         {/* Action Buttons (Floating) - Hidden on mobile because parent is hidden */}
         <div className="absolute top-2 right-2 sm:top-3 sm:right-3 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            {place.websiteUri && (
                <a href={place.websiteUri} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="p-1.5 sm:p-2 bg-white/90 backdrop-blur-sm hover:bg-white text-slate-700 rounded-full shadow-md transition-all hover:scale-110">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 sm:h-4 sm:w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" /></svg>
                </a>
            )}
            <button onClick={handleDelete} className="p-1.5 sm:p-2 bg-white/90 backdrop-blur-sm hover:bg-white text-rose-500 rounded-full shadow-md transition-all hover:scale-110">
      ...
      <div className="p-3 sm:p-5 flex flex-col flex-grow">
        <div className="flex justify-between items-start mb-1">
            <h3 className={`font-bold text-sm sm:text-lg leading-tight transition-colors line-clamp-2 ${isSelected ? 'text-primary-600' : 'text-slate-800'}`}>
                {place.name}
            </h3>
        ...
        <div className="flex items-center text-slate-500 text-xs font-medium mb-0 sm:mb-3">
            ...
            {place.isVerified && (
                <span className="ml-2 flex items-center text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded text-[10px]">
                    ...
                    <span className="hidden sm:inline">已驗證</span>
                </span>
            )}
        </div>

        {/* Description: Hidden on Mobile */}
        <p className="hidden sm:block text-slate-600 text-sm mb-4 flex-grow line-clamp-3 leading-relaxed opacity-90">
            {place.description}
        </p>
        ...
        <div className="hidden sm:flex mt-auto items-center justify-between pt-3 border-t border-slate-50">
